	...
	public void run() {
		try {
			...
			while (mRunning) {
				...
				//if (PeerManager.get(0) == this && Calendar.getInstance().getTimeInMillis() > mLastHeartbeat + 5000) {
		...
		if (PeerManager.get(0) == this) {
			Console.write("서버와의 연결이 종료되었습니다.");
			Main.endWork();
		} else {
			Console.write(this, "연결이 종료되었습니다.");
