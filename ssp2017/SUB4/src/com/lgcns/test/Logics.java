...
public class Logics {
	...
	public void doProcess4(String filename, String inputPath, String report, String logPrefix, String outputPath, String execName, String execPath) {
		HashMap<String, Integer> map = new HashMap<>();
		HashMap<String, LogWriterThread> mapThread = new HashMap<>();
		...
		try {
			while ((msg = br.readLine()) != null) {
				...
				if (!map.containsKey(tokens[1])) {
					...
					LogWriterThread thread = new LogWriterThread(filename, inputPath, logPrefix, outputPath, execName, execPath, tokens[1]);
					mapThread.put(tokens[1], thread);
					thread.start();
				}
			...
			//TODO: wait to all threads closed.
