		...
		if (cmd[0].equalsIgnoreCase("quit")) {
			...
			if (!isCmdError(cmd, 2)) {
				if (cmd[1].equalsIgnoreCase("server")) {
					Console.write("서버 주소: " + PeerManager.get(0).getRemoteAddress() + ":"  + PeerManager.get(0).getRemotePort());
					mCanRead = true;
					return;
			...
			if (!isCmdError(cmd, 4)) {
				...
				try {
					...
					Console.error("잘못 된 포트번호 입니다.");
				} catch (UnknownHostException e) {
					Console.error("잘못 된 호스트 입니다.");
				} catch (IOException e) {
					Console.error("상대 피어에 접속 할 수 없습니다.");
				}
