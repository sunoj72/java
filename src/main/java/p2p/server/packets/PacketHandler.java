package p2p.server.packets;

import java.util.Calendar;
import java.util.Hashtable;
import java.util.Vector;

import p2p.server.p2pserver.Console;
import p2p.server.peers.Peer;
import p2p.server.peers.PeerManager;
import p2p.server.sharing.FileManager;
import p2p.server.sharing.SharedFile;
...
public class PacketHandler {
	...
	public static void doPacket(Peer peer, Packet packet) {
		...
		case TYPE_CHECK_PEER:
			...
			peer.write(packet);
			if (peer.isLeaderPeer()) Console.write(peer, "리더피어입니다.");
			else Console.write(peer, "일반피어입니다.");
			break;
		case TYPE_HEART_BEAT:
			peer.setLastHeartbeat(Calendar.getInstance().getTimeInMillis());
			//peer.write(packet);
			//Console.write(peer, "Heartbeat 수신");
			break;
		case TYPE_REGISTER_PEER:
			if (peer.isInitialized()) {
				...
				case 0:
					...
					packet.setOption((byte) 1);
					Console.write(peer, "사용자 등록: " + packet.getPayload());
					break;
				case 1:
					packet.setType(TYPE_GENERAL_MESSAGE);
		...
		case TYPE_REGISTER_FILE:
			...
			Console.write(peer, "공유 추가: " + msg[0]);
			packet.setOption((byte) 1);
...
//						leadp.write(packet);
		...
		case TYPE_REQUEST_FILE:
			msg = (packet.getPayload()).split("\t");
			Console.write(peer, "전송 요청: " + msg[0] + " to " + msg[1] + ":" + msg[2]);
			break;
		case TYPE_TRANSFER_FILE:
			...
			if (packet.getOption() == 0) {
				...
				Console.write(peer, "전송 시작: " + "(" + msg[0] + ":" + tf.getFileName() + " to " + msg[1] + ":" + msg[2] + "(" + tf.getFileSize() + " bytes)");
				peer.setLoads(peer.getLoads() + 1);
			} else if (packet.getOption() == 2) {
				FileManager.findFile(Integer.parseInt(msg[0]), tHashtable);
				SharedFile tf = null;
				for (Peer tp: tHashtable.keySet()) { tf = tHashtable.get(tp).get(0); break; }
				Console.write(peer, "전송 종료: " + "(" + msg[0] + ":" + tf.getFileName() + " to " + msg[1] + ":" + msg[2] + "(" + tf.getFileSize() + " bytes)");
				peer.setLoads(peer.getLoads() - 1);
			}
			break;
		...
		case TYPE_RELAY_PEERS:
			...
			break;
		default:
			Console.error(peer, "알 수 없는 패킷을 전달받았습니다.");
		}
	}
