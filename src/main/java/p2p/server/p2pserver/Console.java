...
public class Console extends Thread {
	...
	public static boolean openLogger() {
		 try {
			...
			Console.write("로깅 시작: p2p_server.log");
			return true;
		} catch (IOException e) {
			return false;
		}
	...
	public void run() {
		...
		Console.write("콘솔스레드 종료.");
	}

	private void doCommand(String[] cmd) {
		if (cmd[0].equalsIgnoreCase("quit")) {
			Console.write("서버를 종료합니다.");
			Main.endWork();
			return;
				...
				if (cmd[1].equalsIgnoreCase("peers")) {
					...
					if (!isCmdError(cmd, 3)) {
						if (cmd[2].equalsIgnoreCase("all")) {
							StringBuilder sb = new StringBuilder();
							sb.append("전체 공유파일 정보 출력\n");
							sb.append("======================================================================\n");
							sb.append(" 파일  ID |   소 유 자   | 파일 크기 |         파  일  이  름         \n");
							sb.append("======================================================================\n");
							...
							if (p == null) {
								Console.error("해당 피어를 찾을 수 없습니다.");
								mCanRead = true;
								return;
							}
							StringBuilder sb = new StringBuilder();
							sb.append("피어 " + p.getPeerId() + " 공유파일 정보 출력\n");
							sb.append("=======================================================\n");
							sb.append(" 파일  ID | 파일 크기 |         파  일  이  름         \n");
							sb.append("=======================================================\n");
							String form = "  %8s | %9s | %s \n";
